...
pub fn get_schedule_prompt(our: &Address, timezone: &Option<String>, rules: &str) -> String {
    let tz: Tz = timezone
        .as_deref()
        .unwrap_or("UTC")
        .parse()
    ...
    format!(
        ...
        The rules for a successful schedule are currently:
        {rules}
        ...
        If the request is valid, complete and complies with the rules:
        ...
        If the request violates the scheduling rules or appears to be spam:
...
pub fn get_default_rules() -> String {
    "No specific rules for now!".to_string()
}
